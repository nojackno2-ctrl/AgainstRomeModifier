...
internal sealed class TerrainHeightField
{
    ...
    public TerrainHeightField(int width, int height, ReadOnlySpan<byte> samples, float heightScale = 6f)
    {
    ...
    public float TileWidth => Width - 1;
    public float TileHeight => Height - 1;

    public float SampleHeight(float tileX, float tileY)
    {
        float x = Math.Clamp(tileX, 0, TileWidth);
        float y = Math.Clamp(tileY, 0, TileHeight);
