...
- 無盡軍事模式安全組態為 `20..20` 人、`30000 ms` 增援等待與排程週期、同時活躍隊伍上限 `8`。runtime 只有 20 個 NPC-job slots；無條件 gate bypass 已否決。
...
速度、遠程射程、法術效果半徑及祭司施法距離是獨立功能的專屬欄位，不能加回自訂兵種 UI 或 `.artroop` 新格式。一般單位 Sight 仍可自訂，但 `AllUnitsEntireMapVision` 啟用時以 30000 為最終值。
...
原版／平衡數值先形成 fallback，自訂 preset 再逐欄覆蓋。現行新格式只有 `HP,Dmg,VW,AW,Sight,Relt` 六欄；舊九欄 preset 仍可讀取，但速度、射程與法術半徑一定會捨棄，永不套用或重新匯出。祭司 `Sirad` 同時是施法距離閘門，亦會正規化回基線，交由獨立功能管理。共同欄位優先順序固定為 `RangedRange3x`／`SpellEntireMap` 先套用，`AllUnitsEntireMapVision` 最後覆寫 `Sirad=30000`；遠程武器的 3 倍射程欄位仍保留。此最後一層涵蓋 43 個已知戰鬥／領袖／祭司／攻城單位與男女村民、馱馬，建築及未知列不變。所有支援單位的全地圖視野效果已通過遊戲內實機驗證；AI 長時間索敵行為與效能仍未另行驗證。
...
AI Ultimate 的主體是 3 個可獨立控制的體驗型功能：M1 增援規模 (P1 增援人數 + P10 主營轉兵 + P12 村防轉兵)、無盡重生核心 (M2/M3/M4 的排程、敗亡回收與聚落重生生命週期)、M5 AI 開局資源 (P13 聚落開局資源)。原使用者界面的 M6 已提升為獨立 `RomanReinforcementGarrison` 功能「羅馬增援完全移交」：P8 將軍事增援單位數門檻由 4 提高到 70，P9 讓士兵、馱馬與平民全部解除撤退模式並移交村莊，不再依型別走撤退／解編分支。P8/P9 必須原子套用，舊 `EndlessAi.M6` 設定只作為遷移別名。

村莊駐軍配額倍率（功能 id 沿用 `VillageGarrisonQuota3x`，值改為倍率本身：1=停用、UI 下拉可選 ×2/×3/×5/×10）是另一個獨立的實驗功能，不屬於 M1 或無盡重生核心。P20 在 `SYSTEM/CLAK/SCRIPT/Dorfverteidigung.bci` 精確辨識 code-stream offsets `0xD350/0xD37C/0xD3A8/0xD3D4` 的四個 `s_searchImportantPos` 呼叫，將它們改為 opcode 120 的同腳本 helper 呼叫；helper 再呼叫原始 symbol #151，並以已由原生 VM dispatcher 確認的 opcode 34 將回傳數量乘以所選倍率（乘法字面值位於 helper 第 14 個 word，換倍率時原地改寫、不移動程式碼）。這會把四類防守 ImportantPos 的動態 squad 配額各乘 N，保留原比例與 0 值，且不改變 P12 的每支部隊 20 人。偵測要求四個呼叫點與唯一 helper/target 全部精確吻合；helper 內字面值屬支援集合但與目前選擇不同時回報 Legacy 並可原地改寫遷移，其餘混合或未知狀態拒絕寫入。停用時會恢復四個外部呼叫、移除 88-byte helper 並還原 `codeSize`。目前只有靜態與自動測試證據，新開局實機及效能仍待驗證，因此不納入「所有功能開啟」。
...
- 死亡確認計數（`0x1068C`）：20 → 3 個連續 tick（村莊、領袖、村民、成員
  全滅的確認去彈跳）。
- 軍事增援部隊數門檻：4 → 70（`s_searchTeamUnits(team) < 70`）；這不是電腦玩家數或同時活躍隊伍上限。現行 P8 只修改 `0x195F8` 字面值並保持原版 condition tail／`66,0` gate，因此原版 type-4 聚落、建築、資源、領袖、村民與同時僅一支 type-5 增援隊等條件仍存在。舊門檻 8、30、40、兩個精確的舊 bounded gate 與精確的舊無條件 `jmp +272` gate 都會遷移至門檻 70＋原版 gate；其他組合一律判為 `Unknown` 且拒絕寫入。
- completed-job recycle：0 → 1。
- 增援捐贈公式維持原版。
- `v56[party]` 撤退配額隨獨立「羅馬增援完全移交」啟用（舊 `M6` 僅為遷移別名）：10 處匹配點中只修改索引 9（偏移量 `0x17880`，捐贈流程，原值 `[90,15]`）為 `[66,0]`（撤退配額歸零 → 進入 donation 的單位全數移交 type-4 村莊）。索引 8（偏移量 `0x16A44`，`RoemischerNachschub` 初始化，原值 `[90,6]`）**永遠保持原版**：反譯證實該站點寫入的 `v56` 是「士兵生成預算」（內部函式 `0xA964` 於 `0xA9CC` 讀取並在生成迴圈中遞減消耗，`s_randRange(2,4)` 名士兵），歸零會導致增援只生出村民、完全沒有士兵（2026-07-08 實測回報並修復）。舊版「索引 8、9 皆 `[66,0]`」的狀態判為 Legacy，套用時自動把索引 8 遷回 `[90,6]`。其餘 8 處點均保持原版值。
- P9 第三控制點：狀態 49 捐贈走訪內的單位型別過濾。原版在 `0x18238`（全檔唯一的 `s_getUnitType` 呼叫）判斷 `s_getUnitType(obj) == 1` 才進入配額／捐贈分支，士兵小隊（squad，型別 != 1）留在撤退陣列。2026-07-18 新局仍觀察到增援撤退後，依使用者要求把 Ultimate 改為 `jz(117)+0`：吃掉型別分流，讓士兵、駄馬與平民全部進入 donation。舊版 `jz+0` 堆積的原因是 direct mark 未解除 party mode；現行輸出會搭配第四控制點的 release helper。簽章 `[128,214, 73,-2, 86, 66,1, 96,102, ?, ?]`；停用還原 `jz+92`，舊 `jnz+92` 判為 Legacy。
- P9 第四控制點（2026-07-18 修正）：實機複查發現駄馬會經 state 32/34 解編成村民，但舊 `jnz+92` 的士兵在村莊直接消失。原因是 donation action 只呼叫 `s_setObjMark`，士兵仍保留 reinforcement-party 的 script mode 1。新輸出把同一 call site 改成 opcode 120 內部 `callint`，指向附加於 BCI code section 尾端的三參數 helper；helper 先執行 `s_setScriptMode(0, x, y)`，再執行 `s_setObjMark(recipientMark, x, y)`。重新反編譯發現第一版 helper 誤用 opcode 160；VM dispatcher 證實 160 是 typed-array `arrCreate`，不是內部函式 reference，因此該版實機仍會失敗。精確的 160/helper/target 狀態判為 Legacy 並於 Apply 自動遷成 120；helper bytes、target 或其他 opcode 不符則判 Unknown 並拒寫。既有指令位址不移動，只增加 `codeSize` 並安全後移 symbol/variable sections。
- **P9 現行四控制點總結（2026-07-18 新局實機驗證通過）**：① 索引 8 `0x16A44` 維持 `[90,6]`；② 索引 9 `0x17880` 改 `[66,0]`；③ 型別過濾改 `jz(117)+0`，所有單位 fall-through；④ donation action 呼叫 `mode 0 → remark` helper。五份 ENDL 腳本的 apply、idempotence、restore-to-identical-bytes，以及 Save Repair／整體 Apply 整合測試均已通過；使用者在新開無盡局確認士兵、馱馬與平民完整移交村莊且不再撤退。
- gate 保持原版 `66,0`。
...
獨立的實驗性 `CameraZoomOut1` 不修改解析度。遊戲內實測證明原生縮放下限必須從 0 提高到第一個離散級距 1 才會顯示更大戰場；後續嘗試的浮點 0.5 沒有可見作用，因此已撤回。現行補丁把初始化 `0x81388`、讀檔 `0x8D880`、任務腳本 `0x14C1FA` 三個持久化 setter 呼叫導向 `0x1625C0` 的 28-byte cave，把小於 `1.0f` 的要求提升為 `1.0f`，再 tail-jump 至原生 setter；引擎生成快取的暫時縮放不攔截。舊 0.5 cave 會偵測為 `LegacyZoomHalf`，勾選時只改寫 cave 遷移為 +1，取消時也可直接還原。其他混合／未知狀態拒絕啟用；還原先移除三個 redirect，最後清空 cave。+1 的視野擴張已實測成立，但世界中的單位、血條與資訊也會縮小；選取、捲動、霧區、邊界、存讀檔與任務相容性仍待驗證。
