...
`VillageGarrisonQuota3x` 是另一個獨立的實驗功能，不屬於 M1 或無盡重生核心。P20 在 `SYSTEM/CLAK/SCRIPT/Dorfverteidigung.bci` 精確辨識 code-stream offsets `0xD350/0xD37C/0xD3A8/0xD3D4` 的四個 `s_searchImportantPos` 呼叫，將它們改為 opcode 120 的同腳本 helper 呼叫；helper 再呼叫原始 symbol #151，並以已由原生 VM dispatcher 確認的 opcode 34 將回傳數量乘以 3。這會把四類防守 ImportantPos 的動態 squad 配額各乘三，保留原比例與 0 值，且不改變 P12 的每支部隊 20 人。偵測要求四個呼叫點與唯一 helper/target 全部精確吻合；混合或未知狀態拒絕寫入。停用時會恢復四個外部呼叫、移除 88-byte helper 並還原 `codeSize`。目前只有靜態與自動測試證據，新開局實機及效能仍待驗證，因此不納入「所有功能開啟」。
