...
AI Ultimate 的主體是 3 個可獨立控制的體驗型功能：M1 增援規模 (P1 增援人數 + P10 主營轉兵 + P12 村防轉兵)、無盡重生核心 (M2/M3/M4 的排程、敗亡回收與聚落重生生命週期)、M5 AI 開局資源 (P13 聚落開局資源)。原使用者界面的 M6 已提升為獨立 `RomanReinforcementGarrison` 功能「羅馬增援士兵留守」：P8 將軍事增援單位數門檻由 4 提高到 70，P9 使士兵小隊留守、馱馬／平民離場；兩者必須原子套用。舊 `EndlessAi.M6` 設定只作為遷移別名。
...
- 增援捐贈公式維持原版。
- `v56[party]` 撤退配額隨獨立「羅馬增援士兵留守」啟用（舊 `M6` 僅為遷移別名）：10 處匹配點中只修改索引 9（偏移量 `0x17880`，捐贈流程，原值 `[90,15]`）為 `[66,0]`（撤退配額歸零 → 超出配額的單位全數以 `s_setObjMark` 捐給 type-4 隊伍）。索引 8（偏移量 `0x16A44`，`RoemischerNachschub` 初始化，原值 `[90,6]`）**永遠保持原版**：反譯證實該站點寫入的 `v56` 是「士兵生成預算」（內部函式 `0xA964` 於 `0xA9CC` 讀取並在生成迴圈中遞減消耗，`s_randRange(2,4)` 名士兵），歸零會導致增援只生出村民、完全沒有士兵（2026-07-08 實測回報並修復）。舊版「索引 8、9 皆 `[66,0]`」的狀態判為 Legacy，套用時自動把索引 8 遷回 `[90,6]`。其餘 8 處點均保持原版值。
- P9 第三控制點（2026-07-08 新增；2026-07-17 改為反轉並實機確認）：狀態 49 捐贈走訪內的單位型別過濾。原版在 `0x18238`（全檔唯一的 `s_getUnitType` 呼叫）判斷 `s_getUnitType(obj) == 1` 才進入配額/捐贈分支，士兵小隊（squad，型別 != 1）一律留在撤退陣列、由狀態 50 送往地圖出口銷毀。2026-07-08 出貨的作法是把 jz 位移 `92 -> 0`（吃掉條件、所有單位全數捐贈），但這讓補給隊隨行的駄馬（型別 1）也留村、每波堆積（ESAVE_001 實測）。2026-07-17 最終版改為「反轉」：跳躍 opcode `117 (jz) -> 118 (jnz)`、位移保留 `92`——士兵小隊（型別 != 1）落入歸零配額分支全數捐贈留守，駄馬/平民（型別 1）走原版撤退路徑離場（新局實機確認）。簽章 `[128,214, 73,-2, 86, 66,1, 96,102, ?, ?]`（跳躍 opcode 與位移為萬用碼）。停用時還原 jz+92；`jz+0` 出貨狀態判為 Legacy 並自動遷移。
- **P9 最終三控制點總結（2026-07-17 新局實機確認）**：「羅馬增援士兵留守」需三個控制點協同——① 索引 8 `0x16A44` 維持 `[90,6]`（士兵生成預算）；② 索引 9 `0x17880` 改 `[66,0]`（撤退配額歸零）；③ 狀態 49 型別過濾由 `jz(117)+92` 改為 `jnz(118)+92`（士兵捐贈留守，type-1 馱馬／平民離場）。實機確認最終結果：增援出現士兵（非僅村民）、抵達後留在村莊當守軍不撤退、馱馬不再堆積、軍事型 type-4 AI 正常出現。捐贈後守軍行為已實機證實正常，無需改走狀態 32 dissolve 交付路線。
- gate 保持原版 `66,0`。
