...
AI Ultimate 已拆解並重構為 6 個可獨立控制的體驗型功能模組：M1 增援規模 (P1 增援人數 + P10 主營轉兵 + P12 村防轉兵)、M2 加速增援 (P3 增援等待 + P6 排程迴圈優化)、M3 敗亡快速回收 (P4 撤退期限 + P5 死亡判定去彈跳 + P11 村莊拆除延遲)、M4 強制部落生成 (P7 聚落必定生成)、M5 AI 開局資源 (P13 聚落開局資源)、M6 提升守軍數量 (P8 活躍隊伍數上限門檻由 4 提高到 40)。
...
- 增援捐贈公式維持原版。
- `v56[party]` 撤退配額隨「提升守軍數量 (M6)」啟用：當 M6 啟用時，僅將解壓腳本內 10 處匹配點中的索引 8（偏移量 `0x16A44`，初值 6）與索引 9（偏移量 `0x178A4`，初值 15）這兩處真正的增援部隊撤退配額寫入點（屬於 `RoemischerNachschub` 增援的初始化與捐贈流程）修改為 `[66,0]` (歸零不撤退，交接留作守軍)；當 M6 停用時將此兩處還原為原版值（分別為 `[90,6]` 與 `[90,15]`）。其餘 8 處點在套用與還原時均原封不動保持原版值，以消除對軍事型 AI 部隊初始化與其他部隊撤退機制的干擾，徹底修復軍事型 AI 增援時只出現首領而無部隊的 Bug，同時保證增援部隊完全不撤退。
- gate 保持原版 `66,0`。
