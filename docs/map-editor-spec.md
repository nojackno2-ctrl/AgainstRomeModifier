...
- 統一程式 `AgainstRomeModifier.exe` 內的 WinForms 地圖編輯器保留啟動參數 `--game "<path>"` 與 `--map ENDL_NNN`；兩者同時提供時直接開啟該圖，只提供 `--game` 時仍進入統一啟動器。無參數時沿用修改器的 registry 偵測邏輯：`HKCU/HKLM\Software\Against Rome` 的 `Path` 值，fallback `C:\Program Files (x86)\Against Rome`。
...
>
> 實作狀態補充（2026-07-16）：場景檢查器新增暫存式**物件複製與刪除**。複製以既有物件為模板（`SdlSceneObjectAddition` 沿用模板全部欄位、只覆寫 `team`/`pos`），是唯一安全的新增路徑；刪除寫回時由 `RemoveObject` 重新連續編號。`SdlSceneEditService.SaveChanges` 的寫入順序為：值編輯（原始索引）→ 複製（模板欄位在任何刪除前讀取，複製後刪原件等同移動）→ 依索引遞減刪除。結構性儲存後檔內索引已重編，編輯器會從磁碟重讀並重定「還原到本次開啟時」基準。自由新增（從 objdef 清單挑選）仍為後續工作；複製/刪除後的地圖進遊戲載入驗證是使用者 runtime gate。
>
> 實作狀態補充（2026-07-19）：2D／3D 場景新增明確的「移動選取物件」模式。玩家先在場景清單選取既有物件或待複製物件，再於地表拖曳；編輯器把游標落點轉成地圖世界座標與該 SDL 的相對 `pos`，保留物件的 `team`、Y、高度與來源身分。移動只更新記憶體中的 pending state 與場景 marker，按下正常「儲存」後才走既有 `SdlSceneEditService`、`FileRollbackScope` 與 custom-map 驗證寫回。原廠圖、待刪除物件與未選取狀態不可拖曳；自由新增仍未開放。2D／3D 拖曳落點目前以 64×64 tile 中心為安全格點，精確座標仍可在場景欄位輸入。遊戲內載入／位置變化／還原仍是 runtime gate。
