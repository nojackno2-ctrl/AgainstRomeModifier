...
- **Experimental Stat Modifiers**: Entire-map sight for all supported units (`Sirad=30000`, all factions; static patch/test coverage complete, in-game behavior pending), 3× ranged range (with a built-in accuracy fix — doubled impact radius and zeroed lead-aim scatter so shots still land at the longer range), 2× unit movement, entire-map priest casting distance, 3× spell effect radius, and higher projectile arcs.
...
- **Camera Zoom Out 0.5 (Experimental)**: The first zoom-1 build genuinely showed more battlefield, but runtime testing made units and information too small. The revision uses fractional zoom 0.5 for a milder pullback while integer information/LOD consumers retain the stock zoom-0 scale; old +1 installs migrate automatically. Selection, edge scrolling, fog, bounds, and mission compatibility still need retesting.
