...
- New troop presets contain only `HP,Dmg,VW,AW,Sight,Relt`. Speed, ranged distance, spell radius, and priest `Sirad` (casting distance) are owned by independent features; when `AllUnitsEntireMapVision` is enabled, supported-unit Sight is finally overridden to 30000.
...
The current six-property array is `HP,Dmg,VW,AW,Sight,Relt`. Old nine-property presets remain import-compatible, but Speed, Range, and SpellRadius are discarded and never re-exported or applied. Priest Sight is also normalized to its baseline because `Sirad` controls priest casting distance; independent modifiers own these fields. Shared-field precedence applies `RangedRange3x` and `SpellEntireMap` first, then `AllUnitsEntireMapVision` finally writes `Sirad=30000` without reverting the 3x weapon-range columns. This final override covers the 43 supported combat/leader/priest/siege rows plus male/female civilians and packhorses; buildings and unknown rows remain unchanged. The entire-map vision effect for supported units is runtime-verified in game; long-running AI targeting behavior and performance have not been separately verified.
...
AI Ultimate exposes three independent experience controls: M1 reinforcement size (P1 unit count + P10 Town Hall conversion + P12 Dorfverteidigung defense batch), the integrated Endless Respawn Core (the former M2/M3/M4 scheduling, defeat cleanup, and settlement-respawn lifecycle), and M5 AI starting resources (P13 stockpile). The former user-facing M6 is promoted to the standalone `RomanReinforcementGarrison` feature, **Hand Over All Roman Reinforcements**: P8 raises the military-reinforcement unit threshold from 4 to 70 and P9 releases and assigns soldiers, pack horses, and civilians to the village without a type-based retreat split. P8 and P9 remain atomic; legacy `EndlessAi.M6` is migration-only.

The Village Garrison Quota Multiplier (feature id kept as `VillageGarrisonQuota3x` for migration; the value is now the multiplier itself: 1 = disabled, UI dropdown offers x2/x3/x5/x10) is a separate Experimental option, not part of M1 or the Endless Respawn Core. P20 recognizes the four exact `s_searchImportantPos` calls at code-stream offsets `0xD350/0xD37C/0xD3A8/0xD3D4` in `SYSTEM/CLAK/SCRIPT/Dorfverteidigung.bci` and redirects them through opcode-120 internal calls to one appended helper. The helper calls original symbol #151 and multiplies its result by the selected factor with opcode 34, confirmed as signed multiplication in the native VM dispatcher (the multiplier literal sits at word 14 of the helper, so switching multipliers is an in-place literal rewrite that never moves code). This multiplies each of the four dynamic defensive-ImportantPos squad quotas by N while preserving their ratios and zero values; P12's 20 members per newly produced squad remains independent. Detection requires all four exact call sites and the exact helper/target; a helper literal from the supported set that differs from the current selection reports Legacy and migrates via the in-place rewrite, while any other mixed or unknown state is refused. Disable restores the external calls, removes the 88-byte helper, and restores `codeSize`. Only static and automated evidence exists so far; fresh-game runtime and performance verification remain pending, so Enable All excludes it.
...
- The `v56[party]` retreat quota patch (with standalone **Hand Over All Roman Reinforcements**; legacy `M6` is migration-only) modifies ONLY index 9 (donation flow, offset `0x17880`, original `[90,15]`) to `[66,0]`, so every object admitted by the all-unit fall-through is handed to the type-4 village. Index 8 (offset `0x16A44`, `RoemischerNachschub` initialization, original `[90,6]`) is ALWAYS kept vanilla: disassembly proved that this site's `v56` write is the SOLDIER SPAWN BUDGET (`s_randRange(2,4)`), consumed by the spawn loop in internal function `0xA964` (read at `0xA9CC`, decremented per spawned batch). Zeroing it made reinforcements arrive with villagers only and no soldiers (reported and fixed 2026-07-08). The legacy "both sites `[66,0]`" state is detected as Legacy and migrated (index 8 restored to `[90,6]`) on apply. The other 8 signature sites remain untouched.
- P9 third control point: the unit-type filter inside the state-49 donation walk. Vanilla checks `s_getUnitType(obj) == 1` at `0x18238` before entering quota/donation, so soldier squads remain in the retreat array. After a 2026-07-18 fresh-game report still showed retreat, the requested Ultimate output changed to `jz(117)+0`: the jump is consumed and soldiers, pack horses, and civilians all fall through into donation. The older `jz+0` direct-mark output accumulated objects because it did not release party script mode; current output pairs the fall-through with the fourth release-and-remark control point. Disable restores `jz+92`; the former `jnz+92` split is Legacy.
- **P9 current four-control-point summary (fresh-game runtime-verified 2026-07-18)**: **Hand Over All Roman Reinforcements** requires ① index 8 `0x16A44` remains `[90,6]`; ② index 9 `0x17880` becomes `[66,0]`; ③ the type filter becomes `jz(117)+0` so every reinforcement object enters donation; ④ donation calls the opcode-120 `mode 0 -> remark` helper. Exact five-map round-trip, save-repair, and integration verification pass; the user confirmed in a fresh endless game that soldiers, pack horses, and civilians are handed to the village and no longer retreat.
...
- Military-reinforcement unit-count threshold at decompressed `0x195F8`: `4 -> 70`; this is neither an AI-player limit nor the simultaneous active-party cap. A 2026-07-05 `ESAVE_000` snapshot showed the active Roman military settlement had only about nine 20-member units when the original main-house resource conditions stopped later waves; the earlier bounded gate still let transient leader/civilian state suppress subsequent waves. P8 now uses the original condition tail with `teamUnits < 70`. Earlier type-4-settlement, building, and one-active-type-5-party checks remain intact. The old unconditional `jmp +272` remains rejected because it skipped the unit bound and could exhaust job slots. Thresholds 8, 30, and 40 with the original gate, both exact earlier bounded gates, and the exact old unconditional gate are migrated on Apply; every other gate/threshold combination is `Unknown` and is not written.
...
Experimental `NativeWidescreen1920x1080` uses six EXE signatures to replace native 1600x1200 32-bit mode `0x22` with 1920x1080, force `0x22` at startup, and make IGM read the active mode. The stock IGM dialog selector has no `0x22` branch, so the patch reuses its highest-resolution `dlg_igm12_10` layout while retaining the `igm16001200` resource path. The mode-only build had no effect; the following forced build activated a 1920 framebuffer/input space but IGM still read the persisted old mode and rendered only a left-aligned legacy region. The current build recognizes those prior states as `LegacyUnforced` and `LegacyForcedStaleUi` and writes only their missing verified sites. With dgVoodoo2 enabled, the managed config uses `ScalingMode=stretched_ar` and `WindowedAttributes=borderless, fullscreensize`; for customized configs, unrelated values are retained while these two required keys are updated. Fullscreen testing produced a centered 4:3 fallback, but switching back to a normal titled window reproduced the top-left legacy surface and black regions. The current correction ensures customized configs also receive the borderless desktop-sized profile and still needs runtime retesting. This is not a 16:9 expanded world view; mouse boundaries, edge scrolling, dialogs, minimap interaction, and separate camera zoom remain pending.

Final testing showed that neither fullscreen nor windowed presentation corrected the internal legacy surface, so the six-site EXE experiment is now disabled and retained only for historical detection/restoration. The same serialized UI key is displayed as `Centered High Resolution (4:3)`: applying it always restores all six sites to stock and configures dgVoodoo with `FullScreenMode=true`, `ScalingMode=stretched_ar`, `CenterAppWindow=true`, and `FullscreenAttributes=fake`. `WindowedAttributes` stays empty because runtime testing showed that `fullscreensize` compressed desktop-scaled output into a 1280x1024 window; windowed mode now uses and centers the normal stock-sized window. This fallback keeps stock 1600x1200 and makes no native-16:9 or expanded-world-view claim.
...
The independent Experimental `CameraZoomOut1` does not change resolution. Runtime testing proves that the native minimum must reach the first discrete step, 1, before the battlefield visibly pulls back; the later fractional 0.5 experiment had no visible effect and has been withdrawn. Persistent startup (`0x81388`), save-load (`0x8D880`), and mission-script (`0x14C1FA`) calls redirect through the 28-byte cave at `0x1625C0`, which raises requests below `1.0f` to `1.0f` and tail-jumps to the native setter; temporary cache-generation calls remain untouched. The old 0.5 cave is detected as `LegacyZoomHalf` and migrates by replacing only the cave, while disable can restore it directly. Other mixed/unknown states refuse enabling. The +1 battlefield expansion is runtime-proven, but world units, health bars, and information also become smaller; selection, scrolling, fog, bounds, save/load, and mission compatibility still require verification.
