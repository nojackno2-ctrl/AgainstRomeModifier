...
- The `v56[party]` retreat quota patch (with standalone **Keep Roman Reinforcement Soldiers**; legacy `M6` is migration-only) now modifies ONLY index 9 (donation flow, offset `0x17880`, original `[90,15]`) to `[66,0]` — zero retreat quota, so units beyond the quota are handed over via `s_setObjMark` to the type-4 party. Index 8 (offset `0x16A44`, `RoemischerNachschub` initialization, original `[90,6]`) is ALWAYS kept vanilla: disassembly proved that this site's `v56` write is the SOLDIER SPAWN BUDGET (`s_randRange(2,4)`), consumed by the spawn loop in internal function `0xA964` (read at `0xA9CC`, decremented per spawned batch). Zeroing it made reinforcements arrive with villagers only and no soldiers (reported and fixed 2026-07-08). The legacy "both sites `[66,0]`" state is detected as Legacy and migrated (index 8 restored to `[90,6]`) on apply. The other 8 signature sites remain untouched.
- P9 third control point (added 2026-07-08; changed to inversion and in-game confirmed 2026-07-17): the unit-type filter inside the state-49 donation walk. Vanilla checks `s_getUnitType(obj) == 1` at `0x18238` (the file's ONLY s_getUnitType call) before entering the quota/donation branch, so soldier SQUADS (unit type != 1) always stay in the retreat array and are marched to the map exit and destroyed by state 50. The 2026-07-08 ship changed the jz operand `92 -> 0` (fall-through, donate EVERYTHING), but that also kept the supply pack horses (type 1) in the village, piling up every wave (ESAVE_001). Final 2026-07-17: INVERT instead — jump opcode `117 (jz) -> 118 (jnz)`, operand kept `92` — soldier squads (type != 1) fall into the zeroed quota and are all donated (garrison), pack horses/civilians (type 1) take the vanilla retreat path off-map (confirmed in-game on a new endless game). Signature `[128,214, 73,-2, 86, 66,1, 96,102, ?, ?]` (jump opcode and operand wildcarded). Disable restores jz+92; the shipped `jz+0` state is Legacy and auto-migrates.
- **P9 final three-control-point summary (runtime-confirmed in a new game 2026-07-17)**: **Keep Roman Reinforcement Soldiers** requires all three controls together — ① index 8 `0x16A44` remains `[90,6]` (soldier spawn budget); ② index 9 `0x17880` becomes `[66,0]` (retreat quota zero); ③ the state-49 type filter changes from `jz(117)+92` to `jnz(118)+92` (soldiers are donated; type-1 pack horses/civilians leave). Confirmed in-game result: reinforcements arrive with soldiers, stay as active garrison, pack horses no longer pile up, and the type-4 military AI continues to spawn. Donated-squad behavior was confirmed normal, so no state-32 dissolve-delivery rework is needed.
