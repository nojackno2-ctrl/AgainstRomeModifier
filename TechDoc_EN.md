...
- Military-reinforcement unit-count threshold at decompressed `0x195F8`: `4 -> 30`; this is neither an AI-player limit nor the simultaneous active-party cap. A 2026-07-05 `ESAVE_000` snapshot showed the active Roman military settlement had only about nine 20-member units when the original main-house resource conditions stopped later waves; the earlier bounded gate still let transient leader/civilian state suppress subsequent waves. P8 now uses the original condition tail with `teamUnits < 30`. Earlier type-4-settlement, building, and one-active-type-5-party checks remain intact. The old unconditional `jmp +272` remains rejected because it skipped the unit bound and could exhaust job slots. Threshold 8, threshold 40 with the original gate, both exact earlier bounded gates, and the exact old unconditional gate are migrated on Apply; every other gate/threshold combination is `Unknown` and is not written.
