...
namespace AgainstRomeModifier {
    ...
    public static class Loc {
        ...
        private static readonly Dictionary<string, string> Zh = new Dictionary<string, string> {
            ...
            { "GameSpeedTip", "全局縮放遊戲運作時脈。從生產、建造、移動到戰鬥和 AI 反應，通通依所選倍率（2～10 倍）同步加速！掛機等待時極為實用！" },
...
{ "LogSpellRange3xToggled", "法師法術範圍提升 3 倍已{0}。" },
            ...
            { "VillageGarrisonQuota3xTip", "實驗功能：將村莊型 AI 由 OD_IPOS01～04 防守位置動態算出的四類兵團配額各乘以右側選擇的倍率（×2／×3／×5／×10），保留原兵種比例與零值。只提高兵團數；每團人數仍由「增援兵團擴軍」控制。倍率越高，村民、人口、效能與存檔負擔越大，僅建議在新開無盡局測試。" },
            ...
            { "NoRunHpLossTip", "移除戰鬥單位與村民在急行軍（跑步）或士氣歸零時會持續扣減生命值（HP）的限制，讓部隊能夠無限制地全速前進。" },
            ...
            { "CorpseRetentionTip", "將引擎開始回收最早死亡物件前保留的空物件槽，從 500 降為 50，讓戰場可多保留約 450 個物件。此選項不會完全停用回收：固定的 14,000 格物件池仍需空槽給新單位、投射物與存檔流程。長時間大型戰役的效能與穩定性尚待遊戲內驗證。" },
