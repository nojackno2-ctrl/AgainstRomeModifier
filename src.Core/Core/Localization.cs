...
namespace AgainstRomeModifier {
    ...
    public static class Loc {
        ...
        private static readonly Dictionary<string, string> Zh = new Dictionary<string, string> {
            ...
            { "BtnRepairEndlessAi", "修復無盡 AI 計時" },
            { "MsgSelectSaveToRepairAi", "請先選擇要修復的無盡模式存檔。" },
            { "MsgConfirmRepairEndlessAi", "修復前會先建立完整備份，再更新存檔內嵌的無盡 AI 排程。是否繼續？" },
            { "MsgRepairEndlessAiSuccess", "無盡 AI 計時已修復，並已建立修復前備份。" },
            { "MsgRepairEndlessAiAlready", "這份存檔已包含無盡 AI 計時修復。" },
            { "MsgRepairEndlessAiFailed", "修復無盡 AI 計時失敗: " },
            ...
            { "AllUnitsEntireMapVisionTip", "將所有已知戰鬥單位、領袖、祭司、攻城武器、村民與馱馬的視野（Sirad）設為 30000，敵我全陣營一體適用。與遠程射程 3 倍或法師全地圖施法同開時，本功能最後覆寫 Sirad，但不取消武器本身的 3 倍射程；可能改變 AI 索敵行為，尚待遊戲內實機驗證。" },
            { "LogAllUnitsEntireMapVisionToggled", "所有單位全地圖視野已{0}。" },
            { "NativeWidescreen1920x1080", "原生 1920×1080 寬螢幕" },
            { "NativeWidescreen1920x1080Tip", "將原版 1600×1200 32-bit 顯示模式替換為 1920×1080，讓遊戲引擎建立真正的寬螢幕 viewport，而非拉伸既有畫面。請在遊戲解析度選單選擇 1920×1080 32-bit。IGM UI 暫沿用 1600×1200 資源，介面定位、滑鼠點擊、邊緣捲動與小地圖仍待遊戲內驗證。" },
...
{ "LogSpellRange3xToggled", "法師法術範圍提升 3 倍已{0}。" },
            ...
            { "SvcLogNativeWidescreenApplied", "已將原生 1600×1200 32-bit 顯示模式替換為 1920×1080。" },
        ...
        private static readonly Dictionary<string, string> En = new Dictionary<string, string> {
            ...
            { "BtnRepairEndlessAi", "Repair Endless AI Timer" },
            { "MsgSelectSaveToRepairAi", "Please select an endless-mode save to repair." },
            { "MsgConfirmRepairEndlessAi", "A full backup will be created before updating the endless AI scheduler embedded in this save. Continue?" },
            { "MsgRepairEndlessAiSuccess", "The endless AI timer was repaired and a pre-repair backup was created." },
            { "MsgRepairEndlessAiAlready", "This save already contains the endless AI timer repair." },
            { "MsgRepairEndlessAiFailed", "Failed to repair the endless AI timer: " },
            ...
            { "AllUnitsEntireMapVisionTip", "Sets sight (Sirad) to 30000 for all known combat units, leaders, priests, siege engines, villagers, and packhorses across every faction. When combined with 3x ranged range or entire-map priest casting, this feature owns Sirad last without cancelling the 3x weapon range; AI behavior still requires in-game verification." },
            { "LogAllUnitsEntireMapVisionToggled", "Entire-Map Vision for All Units {0}." },
            { "NativeWidescreen1920x1080", "Native 1920x1080 Widescreen" },
            { "NativeWidescreen1920x1080Tip", "Replaces the original 1600x1200 32-bit display mode with 1920x1080 so the engine creates a true widescreen viewport instead of stretching a fixed frame. Select 1920x1080 32-bit in the game's resolution menu. The feature temporarily reuses the 1600x1200 IGM UI resources; UI placement, mouse hit-testing, edge scrolling and the minimap still require in-game verification." },
...
{ "LogSpellRange3xToggled", "3x Spell Effect Radius {0}." },
            ...
            { "SvcLogNativeWidescreenApplied", "Replaced the native 1600x1200 32-bit display mode with 1920x1080." },
