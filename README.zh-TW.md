...
- **實驗性屬性功能**：所有支援單位全地圖視野（`Sirad=30000`、敵我全陣營；靜態 patch／測試已完成，遊戲內行為待驗證）、遠程射程 3 倍（內建命中修正：落點傷害半徑加倍＋預判散布歸零，避免拉遠射程後打不中）、單位移速 2 倍、祭司全地圖施法距離、法術效果半徑 3 倍與拋射彈道增高。
- **實驗性原生寬螢幕**：將原生 1600×1200 32-bit 模式替換為 1920×1080，讓引擎收到真正的寬螢幕 viewport，而非把 4:3 畫面拉伸。套用／偵測／還原與靜態測試已完成；是否真的增加世界可視範圍，以及 UI 定位、滑鼠點擊、邊緣捲動和小地圖行為仍待遊戲內驗證。
- **兵種預設匯出／匯入與一鍵控制**：`.artroop` 新檔格式固定為六欄 `HP,Dmg,VW,AW,Sight,Relt`。舊九欄檔仍可讀取，但已移除的欄位一定會捨棄；另提供一鍵啟用或停用所有已註冊功能。
