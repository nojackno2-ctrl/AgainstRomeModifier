...
`version.dll` here is a **prebuilt binary** of this repository's own
...
- Source: `native/argm-trace` (32-bit `version.dll` proxy, MSVC/Win32).
- Embedded by `src.Core/AgainstRome.Core.csproj` as `argm-trace.version.dll`
...
copy build\argm-trace\Release\version.dll ThirdParty\argm-trace\version.dll
```
