...
namespace AgainstRomeModifier {
    ...
    public static class Loc {
        ...
        private static readonly Dictionary<string, string> Zh = new Dictionary<string, string> {
            ...
            { "AiM6Tip", "無盡模式：新局初始化時把 type-1 村莊型 AI 上限由隨機 2～4 固定為 4。只影響套用後新開的遊戲，不改寫既有存檔。" }
        };
        ...
        private static readonly Dictionary<string, string> En = new Dictionary<string, string> {
            ...
            { "AiM6Tip", "Endless mode: fixes the type-1 village AI cap at 4 instead of randomly choosing 2-4 when a new game initializes. Affects only games started after applying; existing saves are not rewritten." }
        };
    }
}
