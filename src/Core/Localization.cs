...
namespace AgainstRomeModifier {
    ...
    public static class Loc {
        ...
        private static readonly Dictionary<string, string> Zh = new Dictionary<string, string> {
            ...
            { "LogEndlessAiUltimateApplied", "AI終極模式已套用：{0} 個無盡腳本的增援單位人數改為 {1}；完成的增援工作會自動回收，軍事增援冷卻為 5 秒，被擊敗電腦的政黨撤退／清理期限由 10 分鐘縮短為 5 秒（6 處）、死亡確認由 20 tick 縮短為 3 tick；增援輪詢為 5–10 秒，增援生成門檻為 40 支戰鬥單位，並保留原版安全閘門。" },
            ...
            { "AiUltimateModeTip", "無盡模式下電腦增援與被擊敗後的重新到達加速為秒級（增援 5 秒、陣亡名額 5 秒內釋放），大幅增加電腦出兵頻率與挑戰性。" }
        };
        ...
        private static readonly Dictionary<string, string> En = new Dictionary<string, string> {
            ...
            { "LogEndlessAiUltimateApplied", "AI Ultimate Mode applied in {0} endless scripts: reinforcement unit size is {1}, completed jobs recycle, military reinforcement cooldown is 5 seconds, defeated-party cleanup deadlines shrink from 10 minutes to 5 seconds (6 sites), and dead-party confirmation drops from 20 to 3 ticks. Reinforcement polling is 5-10 seconds, the reinforcement-spawn threshold is 40 battle units, and the original safety gate remains enabled." },
            ...
            { "AiUltimateModeTip", "Accelerates AI reinforcements and respawn times to seconds in endless mode, increasing difficulty." }
        };
    }
}
