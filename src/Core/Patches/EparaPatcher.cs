...
/// 上述兩項命中修正皆已整合為射程 3 倍（RangedRange3x）的一部分。
...
public static class EparaPatcher {
    ...
    public static byte[] GetPatchedBytes(byte[] original, bool rangedRange3x) {
        ArgumentNullException.ThrowIfNull(original);
        if (!rangedRange3x) return original;

        // 兩項命中修正一併整合進射程 3 倍：預判散布歸零 + 提高拋射初速，讓箭矢覆蓋放大後的射程。
