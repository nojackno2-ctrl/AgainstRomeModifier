...
/// <summary>
/// Tests for P8_ActiveLimitPatch which controls the active reinforcement party limit.
/// The 41a8bce version of P8 uses limit=40 with the original 5-slot gate as Ultimate state.
...
public sealed class ReinforcementGatePatchTests
{
    ...
    public void P8_applies_a_unit_count_only_bounded_gate_and_restores_it()
    {
        ...
        var patch = new P8_ActiveLimitPatch();
        ...
        Assert.Equal(40, ReadWord(data, 3));
    ...
    [Fact]
    public void P8_migrates_current_limit40_original_gate_and_old_unbounded_gate()
    {
        var patch = new P8_ActiveLimitPatch();

        // limit=40 with original gate is already Ultimate in this version
        byte[] current = BuildFixture(limit: 40, legacyUnbounded: false);
        Assert.Equal(PatchState.Ultimate, patch.Detect(current));
        // Already Ultimate, so Apply(true) should be no-op
        ...
        Assert.Equal(40, ReadWord(unbounded, 3));
    ...
    public void P8_migrates_bounded_gate_from_experimental_build()
    {
        ...
        // if not recognized. With limit=40, this should be detected as Legacy.
        int[] boundedGate = {
            90, 11, 117, 252,
            90, 11, 117, 236,
            90, 11, 117, 220,
