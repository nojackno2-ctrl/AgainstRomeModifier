    ...
    public partial class ModifierForm {
        ...
        private double[] GetBaseStatsForUnit(string key, double origHp, double origDmg, double origVw, double origAw, bool forceBalance = false) {
            if (!forceBalance && !chkBalance.Checked) {
                return new double[] { origHp, origDmg, origVw, origAw };
            }
            ...
            return TroopConfig.CalculateFactionBaseStats(key, faction, tier, utype);
        }
        ...
        private void LoadDefaultStatsData() {
            ...
            try {
                ...
                foreach (string key in TroopConfig.UnitOrder) {
                    ...
                    double finalDefVw = bases[2] + defVwAdd;
                    if (style == "shield") {
                        finalDefVw = Math.Round(finalDefVw * 1.3);
                    }

                    double finalDefAw = bases[3] + defAwAdd;
                    if (style == "shield") {
                        finalDefAw = Math.Round(finalDefAw * 1.15);
                    }
            ...
            try {
                ...
                if (File.Exists(ressPath)) {
                    ...
                    if (mPri.Success) {
                        ...
                        if (cols.Length > 27) {
                            if (cols[24].Trim() != "0" || cols[25].Trim() != "0") noSpell = false;
                        }
                    }
        ...
        private static void GetMeleeAndRangedDmg(string[] cols, string utype, out double meleeDmg, out double rangedDmg) {
            ...
            rangedDmg = 0;
            for (int w = 1; w <= 8; w++) {
                int wAktiIdx = 78 + (w - 1) * 8;
                int wDamIdx = wAktiIdx + 1;
                int wDtypIdx = 199 + (w - 1);
        ...
        private static void GetMeleeAndRangedRelt(string[] cols, string utype, out double meleeRelt, out double rangedRelt) {
            ...
            rangedRelt = 0;
            if (cols[78].Trim() == "1") {
                double.TryParse(cols[84].Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out meleeRelt);
            }
            for (int w = 2; w <= 8; w++) {
                int wAktiIdx = 78 + (w - 1) * 8;
